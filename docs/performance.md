...
Three cached launches of the signed release installer, with its runtime already installed, had a median interactive readiness of 344.74 ms (range 333.06–472.89 ms), median private working set of 43.15 MiB, and median private commit of 81.05 MiB. Each ten-second idle sample recorded 0 ms of CPU time. The measurement uses the same process counters and UI Automation readiness method as the application, targeting the setup page's enabled, visible `ContinueSetup` command. These figures exclude dependency download, deployment, and first-time runtime preparation. The raw samples are in `artifacts/optimization/setup-launch.json`.
